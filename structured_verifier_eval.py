...
FIELD_VALUES = {
    ...
    "dominant_axis": ["acc_x", "acc_y", "acc_z", "gyro_x", "gyro_y", "gyro_z", "uncertain"],
    "dominant_frequency": ["low", "mid", "high", "uncertain"],
    "burstiness": ["smooth", "bursty"],
}
AXIS_TEXT_TO_CODE = {
    "forward-back acceleration trace": "acc_x",
    "side-to-side acceleration trace": "acc_y",
    "vertical acceleration trace": "acc_z",
    "roll-rate gyroscope trace": "gyro_x",
    "pitch-rate gyroscope trace": "gyro_y",
    "yaw-rate gyroscope trace": "gyro_z",
}
...
def parse_evidence_fields_from_text(
    ...
    *,
    ...
    for phrase, code in AXIS_TEXT_TO_CODE.items():
        if phrase in lower:
...
def build_structured_evidence_prompt(record: dict, prompt_style: str = "strict_json") -> str:
    ...
    schema = {
        ...
        "dominant_axis": "one of acc_x/acc_y/acc_z/gyro_x/gyro_y/gyro_z/uncertain",
    ...
    if prompt_style == "terse":
        ...
        instruction = (
            ...
            '{"intensity":"medium","periodicity":"weak","dominant_axis":"gyro_x","dominant_frequency":"high","burstiness":"smooth"}'
        )
    else:
...
def _parse_model_fields(response: str) -> dict[str, str]:
    ...
    for field in FIELDS:
        ...
        if isinstance(value, str) and value in FIELD_VALUES[field]:
            fields[field] = value
    return fields
...
def extract_claim_fields(text: str) -> dict[str, str]:
    ...
    for phrase, value in periodicity_map.items():
        if phrase in lower:
            ...
            break
    for phrase, value in axis_map.items():
        if phrase in lower:
...
def evaluate_structured_grounding(
    ...
    *,
    ...
    if parser_mode == "model_evidence":
        ...
        evidence_fields_list = [_parse_model_fields(response) for response in responses]
    else:
    ...
    for record, evidence_fields in zip(records, evidence_fields_list):
