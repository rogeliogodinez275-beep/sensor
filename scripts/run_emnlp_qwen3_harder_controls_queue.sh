...
MODEL_TAG="${MODEL_TAG:-qwen3_4b_harder}"
...
LOG_PATH="outputs/queue_logs/emnlp_qwen3_harder_controls_queue_${RUN_ID}.log"
STATUS_PATH="outputs/queue_logs/emnlp_qwen3_harder_controls_status.tsv"
LOCK_PATH="outputs/queue_logs/emnlp_qwen3_harder_controls_queue.lock"
FAILURES=0

exec > >(tee -a "$LOG_PATH") 2>&1
ln -sfn "$(basename "$LOG_PATH")" outputs/queue_logs/emnlp_qwen3_harder_controls_launcher.log

if [[ -f "$LOCK_PATH" ]]; then
  old_pid="$(cat "$LOCK_PATH" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "Qwen3 harder controls queue already running with pid=$old_pid"
    exit 0
  fi
fi
echo "$$" > "$LOCK_PATH"
...
wait_for_llm_slot() {
  while ps -ef | grep -E "scripts/run_(qwen_llm_eval|logprob_reranker|structured_verifier_eval)\.py" | grep -v grep >/dev/null; do
    echo "[$(date -Is)] Waiting for active LLM job before Qwen3 harder-control step"
    sleep "$WAIT_SECONDS"
  done
}
...
echo "EMNLP Qwen3 harder controls queue run id: $RUN_ID"
nvidia-smi || true

run_step \
  "pytest_qwen3_harder_controls_queue" \
  "$STEP_TIMEOUT_SHORT" \
  "" \
  "$PYTHON_BIN" -m pytest \
...
echo "[$(date -Is)] Qwen3 harder controls queue finished with failures=$FAILURES"
log_status "qwen3_harder_controls_queue" "FINISH" "failures=$FAILURES"
nvidia-smi || true
exit "$FAILURES"
