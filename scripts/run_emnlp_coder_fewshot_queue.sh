...
LOG_PATH="outputs/queue_logs/emnlp_coder_fewshot_queue_${RUN_ID}.log"
STATUS_PATH="outputs/queue_logs/emnlp_coder_fewshot_status.tsv"
LOCK_PATH="outputs/queue_logs/emnlp_coder_fewshot_queue.lock"
FAILURES=0

exec > >(tee -a "$LOG_PATH") 2>&1
ln -sfn "$(basename "$LOG_PATH")" outputs/queue_logs/emnlp_coder_fewshot_launcher.log

if [[ -f "$LOCK_PATH" ]]; then
  old_pid="$(cat "$LOCK_PATH" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "Coder fewshot queue already running with pid=$old_pid"
    exit 0
  fi
fi
echo "$$" > "$LOCK_PATH"
...
wait_for_llm_slot() {
  while ps -ef | grep -E "scripts/run_(qwen_llm_eval|structured_verifier_eval)\.py" | grep -v grep >/dev/null; do
    echo "[$(date -Is)] Waiting for active LLM job before coder-fewshot step"
    sleep "$WAIT_SECONDS"
  done
}
...
run_direct() {
  ...
  local output_prefix="outputs/coder_llm_${dataset_name}_hard_v3_prompt_fewshot_json"
  wait_for_llm_slot
  run_step \
    "coder_llm_${dataset_name}_hard_v3_prompt_fewshot_json" \
    "$STEP_TIMEOUT_LLM" \
...
run_hybrid() {
  ...
  local direct_rows="$3"
  local output_prefix="outputs/hybrid_regex_coder_${dataset_name}_hard_v3_prompt_fewshot_json"
  run_step \
    "hybrid_regex_coder_${dataset_name}_hard_v3_prompt_fewshot_json" \
    "$STEP_TIMEOUT_SHORT" \
    "${output_prefix}_metrics.json,${output_prefix}_rows.jsonl" \
    "$PYTHON_BIN" scripts/run_hybrid_verifier_eval.py \
      ...
      --output-metrics "${output_prefix}_metrics.json" \
      --output-rows "${output_prefix}_rows.jsonl" \
      --system-name "hybrid_regex_coder_${dataset_name}_hard_v3_prompt_fewshot_json" || true
}

echo "EMNLP coder fewshot queue run id: $RUN_ID"
df -h /root/autodl-tmp || true
nvidia-smi || true

run_step \
  "pytest_coder_fewshot_queue" \
  "$STEP_TIMEOUT_SHORT" \
  "" \
  "$PYTHON_BIN" -m pytest tests/test_qwen_llm_eval.py tests/test_hybrid_verifier_eval.py tests/test_emnlp_coder_fewshot_queue.py -q || true

run_direct ucihar data/benchmark/ucihar_sensorfact_hard_v3_test.jsonl
# outputs/hybrid_regex_coder_ucihar_hard_v3_prompt_fewshot_json_metrics.json
run_hybrid ucihar outputs/axisfix_structured_regex_ucihar_hard_v3_rows.jsonl outputs/coder_llm_ucihar_hard_v3_prompt_fewshot_json_rows.jsonl

run_direct wisdm data/benchmark/wisdm_sensorfact_hard_v3_test.jsonl
# outputs/hybrid_regex_coder_wisdm_hard_v3_prompt_fewshot_json_metrics.json
run_hybrid wisdm outputs/axisfix_structured_regex_wisdm_hard_v3_rows.jsonl outputs/coder_llm_wisdm_hard_v3_prompt_fewshot_json_rows.jsonl

run_direct mhealth data/benchmark/mhealth_sensorfact_hard_v3_test.jsonl
# outputs/hybrid_regex_coder_mhealth_hard_v3_prompt_fewshot_json_metrics.json
run_hybrid mhealth outputs/axisfix_structured_regex_mhealth_hard_v3_rows.jsonl outputs/coder_llm_mhealth_hard_v3_prompt_fewshot_json_rows.jsonl

run_step \
  "emnlp_report_after_coder_fewshot_queue" \
  "$STEP_TIMEOUT_SHORT" \
  "" \
...
echo "[$(date -Is)] EMNLP CODER FEWSHOT QUEUE FINISHED run_id=$RUN_ID failures=$FAILURES"
nvidia-smi || true
exit "$FAILURES"
