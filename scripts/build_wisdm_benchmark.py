...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build WISDM SensorFact benchmarks.")
    parser.add_argument("--workspace", default=".")
    parser.add_argument("--raw-dir", default="data/raw/wisdm")
    parser.add_argument("--url", default=WISDM_URL)
    parser.add_argument("--download", action="store_true", help="Download WISDM if the raw file is absent.")
    parser.add_argument("--window-size", type=int, default=128)
    parser.add_argument("--stride", type=int, default=64)
    parser.add_argument("--max-timestamp-gap", type=int, default=250_000_000)
...
def main() -> None:
    ...
    try:
        find_wisdm_raw_file(raw_dir)
    except FileNotFoundError:
        if not args.download:
            raise
        download_wisdm_dataset(raw_dir, url=args.url)
    ...
    if not train or not test:
        raise RuntimeError(f"WISDM produced train={len(train)} test={len(test)} windows")

    processed_dir = workspace / "data" / "processed"
    benchmark_dir = workspace / "data" / "benchmark"
    write_jsonl(processed_dir / "wisdm_train.jsonl", [row.to_json_dict() for row in train])
    write_jsonl(processed_dir / "wisdm_test.jsonl", [row.to_json_dict() for row in test])
    write_json(
        processed_dir / "wisdm_metadata.json",
        {
            "dataset_id": "wisdm",
            "train_windows": len(train),
            "test_windows": len(test),
            "channels": WISDM_CHANNEL_NAMES,
            "window_size": args.window_size,
            "stride": args.stride,
    ...
    extractor = EvidenceExtractor(
        ...
        channel_names=WISDM_CHANNEL_NAMES,
        sample_rate=args.sample_rate,
    )
    ...
    base_test_path = benchmark_dir / "wisdm_sensorfact_test.jsonl"
    write_jsonl(benchmark_dir / "wisdm_evidence_train.jsonl", train_cards)
    write_jsonl(benchmark_dir / "wisdm_evidence_test.jsonl", test_cards)
    write_jsonl(benchmark_dir / "wisdm_sensorfact_train.jsonl", train_records)
    write_jsonl(base_test_path, test_records)
    write_json(benchmark_dir / "wisdm_evidence_calibrator.json", calibrator.to_json_dict())

    variants = [item.strip() for item in args.hard_variants.split(",") if item.strip()]
    for variant in variants:
        output_path = benchmark_dir / f"wisdm_sensorfact_hard_{variant}_test.jsonl"
        ...
        print(f"Wrote {count} WISDM hard-{variant} records to {output_path}")

    print(
        "WISDM benchmark finished: "
        f"train={len(train)} test={len(test)} channels={','.join(WISDM_CHANNEL_NAMES)}"
    )
