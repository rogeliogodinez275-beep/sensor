...
LOG_PATH="outputs/queue_logs/emnlp_constrained_contrastive_queue_${RUN_ID}.log"
STATUS_PATH="outputs/queue_logs/emnlp_constrained_contrastive_status.tsv"
LOCK_PATH="outputs/queue_logs/emnlp_constrained_contrastive_queue.lock"
FAILURES=0

exec > >(tee -a "$LOG_PATH") 2>&1
ln -sfn "$(basename "$LOG_PATH")" outputs/queue_logs/emnlp_constrained_contrastive_launcher.log

if [[ -f "$LOCK_PATH" ]]; then
  old_pid="$(cat "$LOCK_PATH" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "CONSTRAINED CONTRASTIVE queue already running with pid=$old_pid"
    exit 0
  fi
fi
echo "$$" > "$LOCK_PATH"
...
wait_for_llm_slot() {
  while ps -ef | grep -E "scripts/run_(qwen_llm_eval|structured_verifier_eval)\.py" | grep -v grep >/dev/null; do
    echo "[$(date -Is)] Waiting for active LLM job before constrained-contrastive step"
    sleep "$WAIT_SECONDS"
  done
}
...
run_hybrid() {
  local dataset_name="$1"
  local output_prefix="outputs/hybrid_regex_coder_${dataset_name}_hard_v3_constrained_prompt_contrastive_elim_json_caption_only"
  run_step \
    "hybrid_regex_coder_${dataset_name}_hard_v3_constrained_prompt_contrastive_elim_json_caption_only" \
    "$STEP_TIMEOUT_SHORT" \
    "${output_prefix}_metrics.json,${output_prefix}_rows.jsonl" \
    "$PYTHON_BIN" scripts/run_hybrid_verifier_eval.py \
      --workspace "$WORKSPACE" \
      --structured-rows "outputs/axisfix_structured_regex_${dataset_name}_hard_v3_rows.jsonl" \
      --direct-rows "outputs/coder_llm_${dataset_name}_hard_v3_constrained_prompt_contrastive_elim_json_caption_only_rows.jsonl" \
      --output-metrics "${output_prefix}_metrics.json" \
      --output-rows "${output_prefix}_rows.jsonl" \
      --system-name "hybrid_regex_coder_${dataset_name}_hard_v3_constrained_prompt_contrastive_elim_json_caption_only" || true
}

echo "EMNLP CONSTRAINED CONTRASTIVE queue run id: $RUN_ID"
df -h /root/autodl-tmp || true
nvidia-smi || true

run_step \
  "pytest_constrained_contrastive_queue" \
  "$STEP_TIMEOUT_SHORT" \
  "" \
  "$PYTHON_BIN" -m pytest \
    ...
    tests/test_emnlp_constrained_contrastive_queue.py -q || true

for dataset in ucihar wisdm mhealth; do
  build_subset "$dataset"
  run_caption_only "$dataset" contrastive_elim_json
  run_hybrid "$dataset"
done

run_step \
  "emnlp_report_after_constrained_contrastive_queue" \
  "$STEP_TIMEOUT_SHORT" \
  "" \
...
echo "[$(date -Is)] EMNLP CONSTRAINED CONTRASTIVE QUEUE FINISHED run_id=$RUN_ID failures=$FAILURES"
nvidia-smi || true
exit "$FAILURES"
