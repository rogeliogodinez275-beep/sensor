...
LOG_PATH="outputs/queue_logs/emnlp_structured_prompt_queue_${RUN_ID}.log"
STATUS_PATH="outputs/queue_logs/emnlp_structured_prompt_status.tsv"
LOCK_PATH="outputs/queue_logs/emnlp_structured_prompt_queue.lock"
FAILURES=0

exec > >(tee -a "$LOG_PATH") 2>&1
ln -sfn "$(basename "$LOG_PATH")" outputs/queue_logs/emnlp_structured_prompt_launcher.log

if [[ -f "$LOCK_PATH" ]]; then
  old_pid="$(cat "$LOCK_PATH" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "Structured prompt queue already running with pid=$old_pid"
    exit 0
  fi
fi
echo "$$" > "$LOCK_PATH"
...
wait_for_llm_slot() {
  while ps -ef | grep -E "scripts/run_(qwen_llm_eval|structured_verifier_eval)\.py" | grep -v grep >/dev/null; do
    echo "[$(date -Is)] Waiting for active LLM job before structured-prompt step"
    sleep "$WAIT_SECONDS"
  done
}
...
run_structured_prompt() {
  ...
  local output_prefix="outputs/structured_prompt_${dataset_name}_hard_v3_${prompt_style}"
  wait_for_llm_slot
  run_step \
    "structured_prompt_${dataset_name}_hard_v3_${prompt_style}" \
    "$STEP_TIMEOUT_LLM" \
...
echo "EMNLP structured prompt queue run id: $RUN_ID"
df -h /root/autodl-tmp || true
nvidia-smi || true

run_step \
  "pytest_structured_prompt_queue" \
  "$STEP_TIMEOUT_SHORT" \
  "" \
  "$PYTHON_BIN" -m pytest tests/test_structured_verifier_eval.py tests/test_emnlp_structured_prompt_queue.py -q || true

for prompt_style in strict_json terse chain_then_json fewshot_json; do
  run_structured_prompt ucihar data/benchmark/ucihar_sensorfact_hard_v3_test.jsonl "$prompt_style"
  run_structured_prompt wisdm data/benchmark/wisdm_sensorfact_hard_v3_test.jsonl "$prompt_style"
  run_structured_prompt mhealth data/benchmark/mhealth_sensorfact_hard_v3_test.jsonl "$prompt_style"
done

run_step \
  "emnlp_report_after_structured_prompt_queue" \
  "$STEP_TIMEOUT_SHORT" \
  "" \
...
echo "[$(date -Is)] EMNLP STRUCTURED PROMPT QUEUE FINISHED run_id=$RUN_ID failures=$FAILURES"
nvidia-smi || true
exit "$FAILURES"
