...
LOG_PATH="outputs/queue_logs/emnlp_external_qwen3_position_prior_queue_${RUN_ID}.log"
STATUS_PATH="outputs/queue_logs/emnlp_external_qwen3_position_prior_status.tsv"
LOCK_PATH="outputs/queue_logs/emnlp_external_qwen3_position_prior_queue.lock"
FAILURES=0

exec > >(tee -a "$LOG_PATH") 2>&1
ln -sfn "$(basename "$LOG_PATH")" outputs/queue_logs/emnlp_external_qwen3_position_prior_launcher.log

if [[ -f "$LOCK_PATH" ]]; then
  old_pid="$(cat "$LOCK_PATH" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "External Qwen3 position-prior queue already running with pid=$old_pid"
    exit 0
  fi
fi
echo "$$" > "$LOCK_PATH"
...
run_position_prior() {
  ...
  local output_prefix="outputs/${MODEL_TAG}_position_prior_${dataset_name}_hard_v3_constrained"
  run_step \
    "${MODEL_TAG}_position_prior_${dataset_name}_hard_v3_constrained" \
    "$STEP_TIMEOUT_LLM" \
...
echo "EMNLP external Qwen3 position-prior queue run id: $RUN_ID"
nvidia-smi || true

run_step \
  "pytest_external_qwen3_position_prior_queue" \
  "$STEP_TIMEOUT_SHORT" \
  "" \
...
run_step \
  "summarize_external_qwen3_position_prior" \
  "$STEP_TIMEOUT_SHORT" \
  "docs/external_qwen3_position_prior_summary_2026-05-20.md,outputs/external_qwen3_position_prior_summary_2026-05-20.json" \
  "$PYTHON_BIN" scripts/summarize_position_prior.py \
    --outputs-dir outputs \
    --position-prefix "${MODEL_TAG}_position_prior" \
    --choice-prefix "${MODEL_TAG}_choice_logprob" \
    --output-md docs/external_qwen3_position_prior_summary_2026-05-20.md \
    --output-json outputs/external_qwen3_position_prior_summary_2026-05-20.json || true

echo "[$(date -Is)] EMNLP EXTERNAL QWEN3 POSITION PRIOR QUEUE FINISHED run_id=$RUN_ID failures=$FAILURES"
nvidia-smi || true
exit "$FAILURES"
