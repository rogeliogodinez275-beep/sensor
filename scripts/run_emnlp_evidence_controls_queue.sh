...
LOG_PATH="outputs/queue_logs/emnlp_evidence_controls_queue_${RUN_ID}.log"
STATUS_PATH="outputs/queue_logs/emnlp_evidence_controls_status.tsv"
LOCK_PATH="outputs/queue_logs/emnlp_evidence_controls_queue.lock"
FAILURES=0

exec > >(tee -a "$LOG_PATH") 2>&1
ln -sfn "$(basename "$LOG_PATH")" outputs/queue_logs/emnlp_evidence_controls_launcher.log

if [[ -f "$LOCK_PATH" ]]; then
  old_pid="$(cat "$LOCK_PATH" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "Evidence controls queue already running with pid=$old_pid"
    exit 0
  fi
fi
echo "$$" > "$LOCK_PATH"
...
wait_for_llm_slot() {
  while ps -ef | grep -E "scripts/run_(qwen_llm_eval|logprob_reranker|structured_verifier_eval)\.py" | grep -v grep >/dev/null; do
    echo "[$(date -Is)] Waiting for active LLM job before evidence-control step"
    sleep "$WAIT_SECONDS"
  done
}
...
visible_alternate_rows_for_dataset() {
  local dataset_name="$1"
  echo "outputs/coder_choice_logprob_${dataset_name}_hard_v3_constrained_full_rows.jsonl"
}
...
run_visible_analysis_for_dataset() {
  local dataset_name="$1"
  local primary_rows
  primary_rows="$(primary_rows_for_dataset "$dataset_name")"
  local alternate_rows
  alternate_rows="$(visible_alternate_rows_for_dataset "$dataset_name")"
  local gated_rows="outputs/coder_gated_vote5_choice_logprob_${dataset_name}_hard_v3_constrained_margin2_rows.jsonl"
  local nogate_prefix="outputs/coder_nogate_vote5_choice_logprob_${dataset_name}_hard_v3_constrained_visible"

  run_step \
    "margin_curve_${dataset_name}_visible_choice_logprob" \
    "$STEP_TIMEOUT_SHORT" \
    "outputs/choice_logprob_margin_curve_${dataset_name}.json,outputs/choice_logprob_margin_curve_${dataset_name}.csv,outputs/choice_logprob_margin_buckets_${dataset_name}.csv" \
    "$PYTHON_BIN" scripts/analyze_margin_curve.py \
      --primary-rows "$primary_rows" \
      --alternate-rows "$alternate_rows" \
      --output-json "outputs/choice_logprob_margin_curve_${dataset_name}.json" \
      --output-threshold-csv "outputs/choice_logprob_margin_curve_${dataset_name}.csv" \
      --output-bucket-csv "outputs/choice_logprob_margin_buckets_${dataset_name}.csv" || true

  run_step \
    "failure_taxonomy_${dataset_name}_visible_gated" \
    "$STEP_TIMEOUT_SHORT" \
    "outputs/failure_taxonomy_${dataset_name}_vote5_vs_gated_choice_logprob.json" \
    "$PYTHON_BIN" scripts/analyze_failure_taxonomy.py \
      --primary-rows "$primary_rows" \
      --gated-rows "$gated_rows" \
      --benchmark "outputs/constrained_${dataset_name}_hard_v3_subset.jsonl" \
      --output-json "outputs/failure_taxonomy_${dataset_name}_vote5_vs_gated_choice_logprob.json" || true

  run_step \
    "paired_label_significance_${dataset_name}_visible_gated" \
    "$STEP_TIMEOUT_SHORT" \
    "outputs/paired_label_significance_${dataset_name}_vote5_vs_gated_choice_logprob.json" \
    "$PYTHON_BIN" scripts/paired_label_significance.py \
      --primary-rows "$primary_rows" \
      --challenger-rows "$gated_rows" \
      --output-json "outputs/paired_label_significance_${dataset_name}_vote5_vs_gated_choice_logprob.json" \
      --system-name "paired_label_significance_${dataset_name}_vote5_vs_gated_choice_logprob" || true

  run_step \
    "dev_calibrate_gate_${dataset_name}_visible_choice_logprob" \
    "$STEP_TIMEOUT_SHORT" \
    "outputs/dev_threshold_heldout_${dataset_name}_choice_logprob.json" \
    "$PYTHON_BIN" scripts/calibrate_gate_threshold.py \
      --primary-rows "$primary_rows" \
      --alternate-rows "$alternate_rows" \
      --output-json "outputs/dev_threshold_heldout_${dataset_name}_choice_logprob.json" \
      --dev-modulus 5 \
      --dev-remainders 0 || true

  run_step \
    "nogate_vote5_choice_logprob_${dataset_name}_hard_v3_constrained_visible" \
    "$STEP_TIMEOUT_SHORT" \
    "${nogate_prefix}_metrics.json,${nogate_prefix}_rows.jsonl" \
    "$PYTHON_BIN" scripts/gate_caption_rows.py \
      --primary-rows "$primary_rows" \
      ...
      --output-rows "${nogate_prefix}_rows.jsonl" \
      --min-alternate-margin -1000000 \
      --system-name "coder_nogate_vote5_choice_logprob_${dataset_name}_hard_v3_constrained_visible" || true
}

echo "EMNLP evidence controls queue run id: $RUN_ID"
nvidia-smi || true

run_step \
  "pytest_evidence_controls_queue" \
  "$STEP_TIMEOUT_SHORT" \
  "" \
  "$PYTHON_BIN" -m pytest \
    tests/test_evidence_control_benchmark.py \
    tests/test_gate_calibration.py \
    tests/test_failure_taxonomy.py \
    tests/test_experiment_lock_report.py \
    tests/test_paired_label_significance.py \
    tests/test_summarize_evidence_controls.py \
    tests/test_emnlp_evidence_controls_queue.py -q || true

for dataset in ucihar wisdm mhealth; do
  run_visible_analysis_for_dataset "$dataset"
...
run_step \
  "lock_evidence_first_result_report" \
  "$STEP_TIMEOUT_SHORT" \
  "docs/evidence_first_result_lock_2026-05-20.md" \
  "$PYTHON_BIN" scripts/lock_evidence_first_report.py \
    --outputs-dir outputs \
    --output-md docs/evidence_first_result_lock_2026-05-20.md || true

run_step \
  "summarize_evidence_controls" \
  "$STEP_TIMEOUT_SHORT" \
  "docs/evidence_control_summary_2026-05-20.md,outputs/evidence_control_summary_2026-05-20.json" \
  "$PYTHON_BIN" scripts/summarize_evidence_controls.py \
    --outputs-dir outputs \
    --output-md docs/evidence_control_summary_2026-05-20.md \
    --output-json outputs/evidence_control_summary_2026-05-20.json || true

run_step \
  "emnlp_report_after_evidence_controls_queue" \
  "$STEP_TIMEOUT_SHORT" \
  "" \
...
echo "[$(date -Is)] EMNLP EVIDENCE CONTROLS QUEUE FINISHED run_id=$RUN_ID failures=$FAILURES"
nvidia-smi || true
exit "$FAILURES"
